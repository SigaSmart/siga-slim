        ...
        $colss = [
            ...
            ['name' => 'status', 'dt' => 1,
                'formatter' => function( $d,$row ) {
                    return TagsHtml::status($d)->exibe($row, 'admin/categories');
                }],
            ['name' => 'id', 'dt' => 2,
                'formatter' => function( $d ) {
                    $I_Edit = TagsHtml::i('fa fa-pencil')->exibe();
                    $I_Delete = TagsHtml::i('fa fa-trash')->exibe();
                    $Btns[] = TagsHtml::a(base_url(sprintf("admin/categories/%s/edit", $d)), $I_Edit)->attributes(
                                    [
                                        'class' => 'btn btn-primary btn-flat btn-sm'
                            ])->exibe();
                    $Btns[] = TagsHtml::a(base_url(sprintf("admin/categories/%s/delete", $d)), $I_Delete)->attributes(
                                    [
